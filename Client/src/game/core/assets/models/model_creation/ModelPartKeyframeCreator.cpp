...
aiVectorKey ModelPartKeyframeCreator::GetPositionKeyframe(
    const aiScene *scene, std::string nodeName, int keyFrameIndex) const {
...
std::vector<aiVectorKey> ModelPartKeyframeCreator::GetNodePositionKeyframes(
    const aiScene *scene,
    std
...
std::vector<aiQuatKey> ModelPartKeyframeCreator::GetNodeRotationKeyframes(
    const aiScene *scene,
    std::
    string nodeName) const {
...
std::vector<aiVectorKey> ModelPartKeyframeCreator::GetNodeScalingKeyframes(
    const aiScene *scene,
    std::
    string nodeName) const {
...
std::map<std::string,
         std::vector<aiVectorKey>> ModelPartKeyframeCreator::
GetAnimPositionKeyframes(
    const aiScene *scene) const {
    auto node_name_to_position_keyframes = std::map<std::string,
    ...
    if (scene->mNumAnimations == 0){
        ...
        for (auto i = 0; i < num_meshes; i++){
            ...
            auto node_name = original_name.substr(
                0,
                original_name.length() - 5);
            ...
            node_name_to_position_keyframes[node_name.c_str()] =
                position_keyframes;
//            node_name_to_position_keyframes.insert(
//                {node_name.c_str(),
//                 position_keyframes});
        }
    }
        ...
        for (auto i = 0; i < num_channels; i++){
            ...
            if (num_position_keys > 0)
                node_name_to_position_keyframes[node_name] = position_keyframes;
//                node_name_to_position_keyframes.insert(
//                    {node_name,
//                     position_keyframes});
        }
...
std::map<std::string,
         std::vector<aiQuatKey>> ModelPartKeyframeCreator::
GetAnimRotationKeyframes(
    const aiScene *scene) const {
    auto node_name_to_rotation_keyframes = std::map<std::string,
    ...
    if (scene->mNumAnimations == 0){
        ...
        for (auto i = 0; i < num_meshes; i++){
            ...
            node_name_to_rotation_keyframes[node_name.c_str()] =
                rotation_keyframes;
//            node_name_to_rotation_keyframes.insert(
//                {node_name.c_str(),
//                 rotation_keyframes});
        }
    }
        ...
        for (auto i = 0; i < num_channels; i++){
            ...
            for (auto j = 0; j < num_rotation_keys; j++){
                ...
                rotation_keyframe.mValue.x -=
                    original_rotation_keyframe.mValue.x;
                rotation_keyframe.mValue.y -=
                    original_rotation_keyframe.mValue.y;
                rotation_keyframe.mValue.z -=
                    original_rotation_keyframe.mValue.z;
                rotation_keyframes.push_back(rotation_keyframe);
            }
            if (num_rotation_keys > 0)
                node_name_to_rotation_keyframes[node_name] = rotation_keyframes;
//                node_name_to_rotation_keyframes.insert(
//                    {node_name,
//                     rotation_keyframes});
        }
...
std::map<std::string,
         std::vector<aiVectorKey>> ModelPartKeyframeCreator::
GetAnimScalingKeyframes(
    const aiScene *scene) const {
    auto node_name_to_scaling_keyframes = std::map<std::string,
                                                   std::vector<aiVectorKey>>();
    if (scene->mNumAnimations == 0){
        auto num_meshes = scene->mNumMeshes;
        for (auto i = 0; i < num_meshes; i++){
            ...
            node_name_to_scaling_keyframes[node_name.c_str()] =
                scaling_keyframes;
//            node_name_to_scaling_keyframes.insert(
//                {node_name.c_str(),
//                 scaling_keyframes});
        }
    }
        ...
        for (auto i = 0; i < num_channels; i++){
            ...
            if (num_scaling_keys > 0)
                node_name_to_scaling_keyframes[node_name] = scaling_keyframes;
//                node_name_to_scaling_keyframes.insert({node_name,
//                                                       scaling_keyframes});
        }
    }
